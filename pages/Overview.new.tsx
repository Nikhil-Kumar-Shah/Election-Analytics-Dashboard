/**
 * Election Analytics Dashboard - Overview Page (Alternative Version)
...
export const Overview: React.FC<OverviewProps> = ({ data, filters }) => {
  ...
  const highestPriority = data.length > 0 
    ? Math.max(...data.map(d => d.priority_score)).toFixed(1)
    : '0';
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <FindingCard 
          ...
          text={`${decliningPct}% of constituencies show negative turnout changes from the previous cycle, signaling potential voter apathy or systemic barriers to voting.`}
        />
        <FindingCard 
          number="03"
            ...
            <tbody>
              {topPriority.map((row, index) => (
                <tr key={`${row.state_name}-${row.ac_name}`} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="px-8 py-4 text-right font-mono font-medium text-slate-700">{row.turnout.toFixed(2)}%</td>
                  <td className="px-8 py-4 text-right font-mono text-slate-500">{row.avg_turnout.toFixed(2)}%</td>
