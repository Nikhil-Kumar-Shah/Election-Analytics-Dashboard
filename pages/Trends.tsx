      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
        <div className="mb-8">
          ...
          <p className="text-sm text-slate-500 mt-1 font-medium">
            Top 15 constituencies with most significant turnout changes from previous cycle
          </p>
        </div>
        {changeData.length > 0 ? (
          <div className="space-y-3">
            {changeData.map((item, index) => (
              <div key={index} className="flex items-center gap-4">
                <div className="w-48 text-sm font-medium text-slate-700 truncate">{item.name}</div>
                <div className="flex-1 bg-slate-100 rounded-full h-6 overflow-hidden">
                  <div
                    className={`h-full flex items-center justify-end px-3 text-xs font-bold text-white ${
                      item.isNegative ? 'bg-red-500' : 'bg-emerald-500'
                    }`}
